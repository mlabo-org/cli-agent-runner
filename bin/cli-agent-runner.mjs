...
import { buildRunnerInvocation, resolveConfiguredRunner } from "../lib/runner-registry.mjs";
import { createRunnerStreamAdapter } from "../lib/runner-stream.mjs";

const TOOL = "cli-agent-runner";
const GENERATED_HEADING = "# Generated by cli-agent-runner MVP";
const START = "<!-- cli-agent-runner-mvp:start -->";
const END = "<!-- cli-agent-runner-mvp:end -->";
...
const FEATURE_PROFILE_GUIDANCE = {
  ...
  "workflow.state-safety":
    "Overlay for this assignment instance: protect .cli-agent-runner workflow state identity, append only validated packets, and avoid tracked-state leakage.",
};
...
async function runWithRunner(commandContext, packet, options) {
  return runConfiguredCli(commandContext, packet, options);
}
...
async function runConfiguredCli(commandContext, packet, options) {
  ...
  try {
    ...
    const structuredResult = streamAdapter.resultText();
    if (structuredResult && invocation.resultSource === "stdout") result.stdout = structuredResult;
    const normalized = normalizeRunnerResult(packet, {
      runner: options.runner,
      runnerCommand: invocation.command,
      runnerResultSource: invocation.resultSource,
    ...
    if (publisher) {
      try {
        ...
        guarded.liveConsoleStatus = "failed";
        guarded.liveConsoleFailure = singleLine(error.message);
      }
    }
    return guarded;
  } finally {
...
function normalizeRunnerResult(packet, context) {
  ...
  const outputFile = existsSync(outputPath) ? readFileSync(outputPath, "utf8") : "";
  const summarySource = selectRunnerSummarySource({
    runnerResultSource,
    outputFile,
...
async function liveConsole(args) {
  const port = parsePort(args.port ?? "0", "--port");
  const server = await startLiveConsole({ port });
  console.log(`ok live_console_viewer_url: ${server.viewerUrl}`);
  console.log(`ok live_console_ingest_url: ${server.eventsUrl}`);
  console.log(`ok live_console_host: ${server.host}`);
  console.log(`ok live_console_port: ${server.port}`);
  console.log("ok live_console_owned: false");
  console.log("ok live_console_stop: press Ctrl-C");
...
function selectRunnerSummarySource({ runnerResultSource, outputFile, stdout, stderr, errorMessage }) {
...
function renderReadme(context) {
  return `# CLI Agent Runner Workspace
...
## D-${context.taskId}-005 Nested CLI Agent Runner Preflight Suppression

- accepted: parent-managed child workers must not ask whether to use CLI Agent Runner again or start an independent nested CLI Agent Runner workflow.
...
## Nested CLI Agent Runner Preflight
...
You are a cli-agent-runner worker for task \`${context.taskId}\`.
...
- ${SUPERVISION_NO_INTERRUPT}`;
  if (text.includes(NESTED_CLI_AGENT_RUNNER_PREFLIGHT)) {
    return insertAfterLineMatching(text, new RegExp(`^- ${escapeRegExp(NESTED_CLI_AGENT_RUNNER_PREFLIGHT)}$`, "m"), block, block);
  }
  return `${text.trimEnd()}\n${block}\n`;
}
...
function normalizeTaskSupervision(text) {
  ...
  if (text.includes("nested CLI Agent Runner preflight suppression rule")) {
    return insertAfterLineMatching(text, /- Each generated child-worker prompt, assignment, handoff, and runner packet carries the nested CLI Agent Runner preflight suppression rule\./m, block, block);
  }
  return `${text.trimEnd()}\n${block}\n`;
}
...
function normalizeRunnerPreambleSupervision(text) {
  ...
  const normalizedPreamble =
    !textHasAnySupervisionSchemaField(preamble) && preamble.includes(NESTED_CLI_AGENT_RUNNER_PREFLIGHT)
      ? insertAfterLineMatching(
        preamble,
        new RegExp(`^${escapeRegExp(NESTED_CLI_AGENT_RUNNER_PREFLIGHT)}$`, "m"),
        renderSupervisionFields(),
        renderSupervisionFields(),
      )
...
function assertMachineRunnableScope(packet, cwd) {
  ...
  if (!prefixes) {
    throw new CliError(
      `run --runner <id> requires a machine-checkable path-only scope; use scope:v1 all, scope:v1 paths=<comma-separated prefixes>, or a legacy simple path-only scope; got scope: ${packet.scope}`,
      1,
    );
  }
  return prefixes;
...
function assertNoDirtyPathsOutsideMachineScope(cwd, scope, prefixes) {
  ...
  if (dirtyOutsideScope.length) {
    throw new CliError(
      `run --runner <id> refuses to launch with dirty files outside scope ${scope}: ${dirtyOutsideScope.join(", ")}`,
      1,
    );
  }
}
...
- nested_cli_agent_runner_preflight: ${NESTED_CLI_AGENT_RUNNER_PREFLIGHT}
...
- runner_command: ${result.runnerCommand}
- runner_result_source: ${result.runnerResultSource}
- runner_stream_format: ${result.runnerStreamFormat}
- runner_config_sources: ${result.runnerConfigSources.join(" -> ")}
- live_console_status: ${result.liveConsoleStatus}
- live_console_run_id: ${result.liveConsoleRunId}
- live_console_failure: ${result.liveConsoleFailure}
...
function appendRunnerEntry(commandContext, heading, entry) {
  ...
  const initial = `# CLI Agent Runner Runner
...
function hasPathEvidence(value) {
  return /(?:^|\s)(?:[.~]?\/|[A-Za-z0-9_.-]+\/[A-Za-z0-9_.\/-]+|\.cli-agent-runner\b|docs\/codex\b|bin\/|tests?\/|src\/|\.mjs\b|\.md\b|\.json\b)/i.test(String(value || ""));
}
...
function printHelp() {
  console.log(`cli-agent-runner MVP CLI
...
Usage:
  ...
  node bin/cli-agent-runner.mjs handoff [--cwd <path>] [--target-cwd <path>] --task-id <id>
  node bin/cli-agent-runner.mjs doctor [--cwd <path>] [--target-cwd <path>]
  node bin/cli-agent-runner.mjs --help

Commands:
  intake   Create or update target .cli-agent-runner workflow files.
...
State:
  ...
  State writes add .cli-agent-runner/ to .git/info/exclude; .gitignore is not edited.
...
function parsePort(value, flag) {
  ...
  if (!Number.isSafeInteger(parsed) || parsed < 0 || parsed > 65535) {
    throw new CliError(`invalid ${flag}: expected an integer from 0 to 65535`, 1);
  }
  return parsed;
}
