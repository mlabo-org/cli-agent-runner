...
export async function startLiveConsole(options = {}) {
  ...
  const codeFontSize = await resolveCodeFontSize(options);
  ...
  const authQuery = new URLSearchParams({ token }).toString();
  const viewerQuery = new URLSearchParams({ token, codeFontSize: String(codeFontSize) }).toString();
  ...
  return {
    ...
    token,
    codeFontSize,
    viewerUrl: `${baseUrl}/?${viewerQuery}`,
    ingestUrl: `${baseUrl}/api/events`,
    snapshotUrl: `${baseUrl}/api/snapshot?${authQuery}`,
    eventsUrl: `${baseUrl}/api/events?${authQuery}`,
    publish: appendEvent,
...
export function parseCodexCodeFontSize(configText) {
  ...
  for (const rawLine of configText.split(/\r?\n/)) {
    ...
    const valueMatch = /^codeFontSize\s*=\s*([+-]?(?:\d+(?:\.\d+)?|\.\d+))\s*(?:#.*)?$/.exec(line);
    if (!valueMatch) continue;
    return normalizeCodeFontSize(Number(valueMatch[1]));
  }
  return null;
}

async function resolveCodeFontSize(options) {
  if (options.codeFontSize !== undefined) {
    return normalizeCodeFontSize(options.codeFontSize) ?? DEFAULT_CODE_FONT_SIZE;
  }
  const codexRoot = process.env.CODEX_HOME?.trim() || path.join(os.homedir(), ".codex");
  const configPath = options.codexConfigPath ?? path.join(codexRoot, "config.toml");
  try {
    return parseCodexCodeFontSize(await readFile(configPath, "utf8")) ?? DEFAULT_CODE_FONT_SIZE;
  } catch {
    return DEFAULT_CODE_FONT_SIZE;
  }
}

function normalizeCodeFontSize(value) {
  const size = Number(value);
  if (!Number.isFinite(size) || size < MIN_CODE_FONT_SIZE || size > MAX_CODE_FONT_SIZE) return null;
...
function updateRunMetadata(run, event) {
  const data = event.data;
  if (!data || typeof data !== "object" || Array.isArray(data)) return;
  if (typeof data.parentRunId === "string" && data.parentRunId.trim()) {
    run.parentRunId = data.parentRunId;
  }
  if (Number.isSafeInteger(data.depth) && data.depth >= 0) run.depth = data.depth;
  if (data.delegationMode === "leaf" || data.delegationMode === "local_orchestrator") {
    run.delegationMode = data.delegationMode;
  }
  if (typeof data.focusScope === "string" && data.focusScope.trim()) {
    run.focusScope = data.focusScope;
  }
}
