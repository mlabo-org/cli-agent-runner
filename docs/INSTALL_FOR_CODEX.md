# Install CLI Agent Runner with Codex
...
Install the authoritative `cli-agent-runner` source through the standard personal marketplace and the official Codex CLI. Preserve unrelated user configuration. Never edit an installed plugin cache directly.
...
```text
<user-home>/plugins/cli-agent-runner
```
...
- `.codex-plugin/plugin.json` names `cli-agent-runner` and has a version.
- The current request authorizes local plugin installation and the required personal-marketplace write.

The bundled `codex-cli` profile requires an authenticated Codex CLI at runtime. The `claude-cli` and `grok-cli` profiles are optional and require their corresponding authenticated CLIs only when selected. Missing optional providers do not block plugin installation.
...
1. Read `.codex-plugin/plugin.json`, this file, and the installation section of `README.md`.
...
4. Confirm that the JSON status is `ready`. A source-path collision, duplicate plugin entry, malformed marketplace, conflicting plugin source, unsupported Node version, or marketplace-name collision is a blocker.
...
6. Tell the user to restart Codex and start a new task so the manifest and skill are loaded. Do not claim the current task hot-reloaded the plugin.
7. In the new task, use this non-executing verification prompt: `CLI Agent Runner の利用条件、標準 runner、Live Console の既定動作を説明して。CLI worker はまだ起動しないで。`
