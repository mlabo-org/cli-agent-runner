...
function printHelp() {
  ...
  node scripts/migrate-legacy-cli-agent-runner-state.mjs [options]

Safe migration for legacy CLI Agent Runner state directories.
...
Default behavior:
  ...
  - copy plans target <git-root>/.cli-agent-runner/ and never delete legacy directories
...
function discoverRepoPlans(args, inputs, audit) {
  ...
  for (const repo of Array.from(reposByRoot.values()).sort((a, b) => a.root.localeCompare(b.root))) {
    ...
    plans.push({
      ...
      targetDir: path.join(repo.root, ".cli-agent-runner"),
      backupDir: "",
...
function excludePatternsFor(legacyDirs) {
  const patterns = [".cli-agent-runner/", ".cli-agent-runner-migration-backups/"];
  for (const legacyDir of legacyDirs) {
...
function applyPlan(plan, runId) {
  try {
    plan.backupDir = path.join(plan.repoRoot, ".cli-agent-runner-migration-backups", runId);
...
function createPreflightBackup(plan) {
  ...
  if (existsSync(plan.targetDir)) {
    copyTree(plan.targetDir, path.join(plan.backupDir, "existing-dot-cli-agent-runner"), {
      onSkip: (message) => plan.warnings.push(`backup skip: ${message}`),
    });
  }
...
function addExcludePatterns(plan) {
  ...
  const block = `${prefix}# cli-agent-runner legacy state migration\n${additions.join("\n")}\n`;
...
function copyLegacyContents(context) {
  ...
  context.actions.push(
    `copied ${context.label} -> .cli-agent-runner/ (new=${copied}, identical=${identical}, conflicts-preserved=${conflicts}, skipped=${skipped})`,
  );
}
...
function renderAuditReport(audit) {
  const lines = [];
  lines.push("# CLI Agent Runner Legacy State Migration Audit");
  lines.push("");
  ...
  if (audit.repositories.length === 0) {
    ...
    for (const plan of audit.repositories) {
      ...
      lines.push(`- target: ${slash(path.relative(plan.repoRoot, plan.targetDir)) || ".cli-agent-runner"}`);
...
function plannedActions(plan) {
  ...
  actions.push(`would copy legacy contents to .cli-agent-runner/ without overwriting divergent files`);
