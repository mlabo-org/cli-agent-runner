...
description: >-
  Run Grok, Claude, Codex, or custom CLI workers with CLI Agent Runner and its default-on IAB Live Console before worker launch. Triggers: CLI Agent Runner, Live Console, Grok/Claude CLI, runner JSON, or .cli-agent-runner continuation. Only explicit silent/no-console requests disable it; excludes official subagents.
---
...
- `run --runner <id>` resolves one validated runner profile, launches it with the jobsite as cwd, captures its configured result source plus process stdout/stderr, applies the declared scope guard, and derives the `process-runner-result` status, summary, exit code, signal, timeout, and failure fields. Direct CLI use starts and retains an owned Live Console by default when no prestarted URL or explicit OFF flag is supplied; `--live-console` remains an optional explicit compatibility spelling.
- The normal plugin-owned run reuses the already visible standby console through `run --runner <id> --live-console-url <tokenized-loopback-url>`. Do not create a second viewer for the same skill activation.
- The parent Codex session owns the persistent standalone console terminal, immediate supported IAB opening, URL injection into later runs, final observation, and cleanup when the task ends or the user explicitly stops it. Do not call private Codex GUI IPC.
...
- An exit-zero, in-scope `run --runner` result is terminal. Store one minimal `process-runner-result` with `status: completed`, runner identity, exit code, and summary, then stop verification for that run.
- Do not evaluate a successful process result with a separate worker-report conformance contract, mark it parent-acceptance-pending, require a follow-up `collect`, or chain `finalize`, `verify-assignments`, `doctor`, or another validator after success. Explicitly requested collection/finalization workflows remain separate commands, not automatic post-run gates.
...
- `collect` and `finalize` remain available for explicitly requested manual workflow-state collection and task-wide coverage. They are not required after a successful `run --runner` result.
...
- Runner profiles may declare an executable-validated `defaultHierarchyDepth`. It is the parent-owned maximum descendant permission when a run omits hierarchy CLI fields, not an instruction that the worker must delegate. The parent also owns concurrency, time, budget, operation-scope, and permission-inheritance boundaries. The assigned worker decides whether descendants materially help within those limits, and CLI Agent Runner passes that policy through without independently deciding that delegation is unnecessary. Model suitability belongs in explicit profile configuration, including `defaultHierarchyDepth: 0`, rather than in a hidden per-run parent judgment. The bundled `grok-cli` profile permits one direct-child level, while bundled Codex and Claude profiles retain zero descendant depth.
...
- Keep that one console visible and ready while target intake, assignment construction, and other pre-run work proceed. Launch each later scoped worker with `--runner <id> --live-console-url <url>` so activity appears in the already open viewer.
...
- The gate shapes the producer assignment for actual repair work; it is not a post-success report validator and never reopens a successful `run --runner` result.
...
- This gate applies only when the parent explicitly invokes the manual `finalize` workflow. It is not part of the normal successful `run --runner` path.
...
- Completed, blocked, failed, or otherwise terminal worker output is an observed result, not heartbeat silence. A successful `run --runner` result stops immediately; explicitly managed collection workflows record lifecycle disposition only for their own collected results.
...
5. Run intake with explicit isolation keys and optional semantic work metadata:
   `node /Users/suzukimakoto/plugins/cli-agent-runner/bin/cli-agent-runner.mjs intake --target-cwd <jobsite> --work-type <auto|documentation|source-change|debug> --task <task> --task-id <id> --epoch <epoch> --scope <scope>`.
6. Run doctor:
   `node /Users/suzukimakoto/plugins/cli-agent-runner/bin/cli-agent-runner.mjs doctor --target-cwd <jobsite>`.
7. Print handoff when needed:
   `node /Users/suzukimakoto/plugins/cli-agent-runner/bin/cli-agent-runner.mjs handoff --target-cwd <jobsite> --task-id <id>`.
8. If pre-existing `.cli-agent-runner` state lacks the debugging integrity gate, run dry-run first:
   `node /Users/suzukimakoto/plugins/cli-agent-runner/bin/cli-agent-runner.mjs normalize-debugging-integrity --target-cwd <jobsite>`.
   Apply only after confirming the target state directory:
   `node /Users/suzukimakoto/plugins/cli-agent-runner/bin/cli-agent-runner.mjs normalize-debugging-integrity --target-cwd <jobsite> --execute`.
9. For `assign`, `collect`, `finalize`, `run`, or `orchestrate`, record operational packets in the jobsite repository's `.cli-agent-runner/runner.md`. Use `--feature-profile <id>` only as an optional scoped overlay for that assignment instance, and keep missing profiles as `feature_profile: none`. Use `--work-type <id>` only as semantic command metadata for gate classification, and keep missing work types as `work_type: auto`.
   ...
   Unless the user explicitly selected silent/no-console/OFF operation, the standalone `live-console --port 0` session and IAB viewer must already be active from skill selection. Reuse its tokenized URL with `run --runner <id> --live-console-url <url>` and keep the console available until the task ends. For direct CLI use without a prestarted URL, `run --runner <id>` owns a console by default. Use `--no-live-console` or `--silent` only for the explicit OFF path. This is a plugin-owned route, not a separate AgentScope dependency.
   When `run --runner <id>` exits zero and the scope guard passes, the process result is complete and terminal. Store the minimal completed result and do not invoke `collect`, `finalize`, `verify-assignments`, `doctor`, or another post-success validator.
10. Ensure generated assignments and runner prompts carry the known producer requirements before launch. A successful `run --runner` result bypasses every collection, lifecycle, Contract Coverage, and metacognitive post-success gate. Failed or blocked output bypasses the quiet-worker stale path and enters only cause-bound failure handling.
...
11. For failure, blocker, stale-premise, or scope-change handling in an explicitly managed collection workflow, record `state_retired` or `continuation_expected` as appropriate. Do not apply this step after a successful `run --runner` result.
12. Run only the verification required by the current user request or the selected explicit manual workflow. A successful `run --runner` result does not authorize task-finalization Contract Coverage or another acceptance pass.
