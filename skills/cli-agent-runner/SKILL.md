...
description: >-
  Run one CLI worker or parallel independent leaf jobs with Grok, Claude, Codex, or custom runners and a default-on IAB Live Console. Triggers: CLI Agent Runner, Live Console, Grok/Claude CLI, runner JSON, or .cli-agent-runner continuation. Only explicit silent/no-console requests disable it; excludes official subagents.
---
...
- The primary execution routes are `run --runner <id>` for exactly one worker and `orchestrate --runner <id> --jobs-file <json>` for parent-declared independent leaf jobs. Bundled IDs are `codex-cli`, `claude-cli`, and `grok-cli`; user-defined IDs use the same execution and result-normalization path.
