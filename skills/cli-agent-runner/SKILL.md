...
- Live Console transport state is separate from child execution, artifact acceptance, and result-contract status; a mid-run viewer transport failure must remain explicit rather than being reported as child failure or hidden by fallback.
...
- Runner profiles may declare an executable-validated `defaultHierarchyDepth` used only when a run omits every hierarchy CLI flag. The bundled `grok-cli` profile defaults to one level, while bundled Codex and Claude profiles retain zero descendant depth. Any explicit hierarchy flag overrides the selected profile default.
...
- The source-change trigger is not optional once CLI Agent Runner has been selected for coding work. Do not let keyword judgment, task smallness, immediate token/latency cost, or apparently simple edits classify source/config/test/canonical/refactor work as non-gate work.
- For source-change work, CLI Agent Runner must check whether a mature GitHub/npm solution should be reused before implementing core logic manually. If reuse is not taken, record why.
...
- For `run|orchestrate --runner grok-cli`, omission of all hierarchy flags resolves through the bundled profile to `hierarchy_mode: one_level`, `max_depth: 1`, `depth: 0`, and `remaining_depth: 1`. Direct Grok children therefore receive no further delegation depth. `--hierarchy-mode none` is the explicit opt-out. Other bundled runners retain the zero-depth default, and any explicit hierarchy flag takes precedence over a runner-profile default.
...
14. Append audit results to `.cli-agent-runner/audit.md`, including checks not run and why. Include Coding Conduct Gate checks. For gate-required work, include context-impact inspection and cross-feature checks or explicitly record why those checks were skipped.
