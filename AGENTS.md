...
This file is the scoped `AGENTS.md` SSOT for one decision: an explicit request to install, set up, or activate this repository as a Codex plugin. It is an execution contract, not general development guidance. Higher-priority system, developer, user, and ancestor instructions still apply.

## Narrow scope

- Trigger this contract only when the current user explicitly asks to install, set up, or activate `cli-agent-runner` from this repository.
